...
baset = mint+round((maxt-mint)*0.1); % set base temperature 
                                     % accorinding to heat range 
                                     % (default is 10%)
...
% initial angle from iniangle2.m function, each bot aims for active region
...
% calculate distance from mo'th previous location and record in bot history
...
% build area to check for edges based on suggested step size
            
bl(it+1,(i*10)-4) = stepcheck(steparea,bl(it+1,(i*10)-6),bl(it,(i*10)-4),bl(it-1,(i*10)-4));
% retrieve step size from stepcheck function (1 if edge encountered
% over step size, unchanged if no obstacle (edge) found
