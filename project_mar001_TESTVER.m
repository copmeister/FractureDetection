...
hold on
martyr(all(~martyr,2),:)=[];
%de = unique(martyr,'rows');

%plot(de(:,2),de(:,1),'b.','MarkerSize',20)
...
%for i = 1:it
    %for j = 1:nb
    %plot(bl(i,(j*10)-8),bl(i,(j*10)-9),'r.','MarkerSize',20)
    %end
%end

%plot(bl(it,2),bl(it,1),'g.','MarkerSize',20)
